...
\bigskip
\newpage
\textbf{Countable Inference Theorem} \; \textit{A set of inequivalent countable functions} $ A^{*} $ \textit{can be weakly inferred by a device if each function $ A_i \in A^{*} $ is independently inferrable. } \\
...
\bigskip
\bigskip
\textbf{Lemma} \; \textit{For every countable function $ \Gamma $ there exists a device $ \mathcal{D} $ such that $ \mathcal{D} $ physically knows all $ \gamma \in \Gamma(U) $ over $ W \subseteq U $ if $ |\Gamma^{-1}(\gamma)| \geq 2 $ for all $ \gamma \in \Gamma $.} \\

\textbf{Proof} \; Take $ U := \N $ and $ W = U $. Choose any $ \Gamma $ as described in the claim. Enumerate $ \gamma \in \Gamma(W) $ as $ 1, 2, \dots, i, \dots $ . Define $ V^{i} = \{u : \Gamma^{-1}(\gamma) = u \} $ for each $ i \in \Gamma(W) $. Similarly enumerate each $ u \in V^{i} $ as $ i_1, i_2, \dots, i_j, \dots $ . Now define the device $ (X, Y) $ as 
...
Define $ \xi : \Gamma(U) \rightarrow \overline{X} $ by $ \xi(i) = \{i_1\} \cup \{j_2: i \neq j \} $. Then for $ \gamma \in \Gamma(W) $, $ u \in \xi(\gamma)$, $ \delta_{\gamma}(\Gamma(u)) = Y(u) $ Furthermore, for each $ \gamma \in \Gamma(W) $, $ \exists u, v \in \xi(\gamma) : u \in (W \subseteq Y^{-1}(1)), v \in (W \subseteq Y^{-1}(-1) $. Hence, $ \mathcal{D} $ physically knows $ \gamma \in \Gamma(U) $. $ \hfill \qed $ \\

\textbf{Lemma} \; \textit{For any set of independently physically knowable characteristic functions $ A^{*} $ there is a device $ \mathcal{D} $ that knows all $ A \in A^{*} $ over a given subset $ W \subseteq U $. } \\
\textbf{Proof} \;  
...
\textbf{Proof} \; Fix any convenient $ A^{*} $ and choose any subset $ A \subseteq A^{*} $ with cardinality $ m $. Let $ U := \B^{*} $. Let $ X: U \rightarrow \B^{*} $ and $ Y:U \rightarrow \{-1, 1\} $ be the $ X $ and $ Y $ defined in the Countable Inference Theorem when given $ A $. $ X $ in this case maps $ \N \rightarrow \B^{*} $ instead of $ \N \rightarrow \N $ but the mappings are equivalent. Then $ \mathcal{D} = (X, Y) > A $ by construction. Take $ d\mu(u) = \ell(u) $ where $ \ell(b) $ is the length of a bit string $ b $. 
