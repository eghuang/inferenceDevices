...
\textbf{Incompressibility Theorem} \quad \textit{Let c be a positive integer. For each fixed y, every finite set A of cardinality m has at least} $ m(1 - 26^{-c}) + 1 $ \textit{elements x with} $ \mathcal{C}(x|y) \geq \log m - c $. \\
...
\bigskip
\bigskip
\textbf{Theorem} \quad \textit{Let c be a positive integer. Every finite set A of cardinality m has at least} $ m(1 - 26^{-c}) + 1 $ \textit{elements x such that} $ \mathcal{C}_\mathcal{D}( x ) \geq \log m - c $. \\


\textbf{Proof} \quad Let $ U := \B^{*} $ and take $ d\mu(u) = l(u) $ where $ l(b) $ is the length of a bit string $ b $. Since $ A $ is finite, we can map its elements to a set of finite bit strings. The number of $ u $ with inference complexity of less than $ \log m - c $ is
