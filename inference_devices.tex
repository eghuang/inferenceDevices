...
\textbf{Countable Inference Lemma} \; \textit{A set of countable functions} $ A^{*} $ \textit{can be weakly inferred by a device} $ \mathcal{D} $ \textit {if for all} $ F \in A $,  $\exists f \in F : |F^{-1}(f)| \geq 2 $. \\
\textbf{Proof} \; We proceed by induction. Let the base case be $ |A^{*}| = 1 $. The base case holds by Theorem 5.1. Assume that the claim holds for some $ n $ such that $ |A^{*}| = n $. We show that it holds for the case of $ n + 1 $. Let $ U := \N $ and denote the device inferring $ A^{*} $ as $ (X, Y) $. Add a new function $ A $ to $ A^{*} $ such that $\exists a \in A(U) : |A^{-1}(a)| \geq 2 $ and $ A $ is not already a member of $ A^{*} $. Enumerate $ a \in A(U) $ as $ 1, 2, \dots, i, \dots, n $ and define $ V^{i} = \{u: A^{-1}(i) = u \} $. Then enumerate each element in $ V^i $ as $ i_1, i_2, \dots, i_j, \dots, i_m $. Then define the inference device $ ( X', Y' ) $ as 
